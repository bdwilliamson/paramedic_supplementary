...
inv_min_ex_zero <- function(vec) 1/min(vec[vec>0]) # inverts the minimum non-zero value
relative_abundances <- rsm[, 7:182]/100
relative_abundances %>% apply(1, inv_min_ex_zero) # mostly whole numbers, good; this gives us the number of reads?
# if the lowest abundance taxon was observed once, and relative_abundances is a percentage,
# then percentage = w / m, so m = 1 / percentage (assuming w = 1 for lowest abundance)
ms <- relative_abundances %>% apply(1, inv_min_ex_zero)
median(ms) # paper says median 4345 reads, 4176 is close

# matrix of counts -- each row replicated across columns
...
# these are samples with non-integer values
...
# and it's only a small number of samples
problematic_samples$id %>% unique

### 500019782501 (why multiply by 6??)
...
# write to file
all_data %>%
  write_csv("data/RSMp01_counts.csv")
