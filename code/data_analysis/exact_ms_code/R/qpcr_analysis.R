...
library("argparse")
library("Cairo")
# remotes::install_github("statdivlab/paramedic", ref = "v0.0.2") # local
...
samp <- if (args$sample_num == nrow(br16_mat)) {1:nrow(br16_mat)} else {sample(1:nrow(br16_mat), args$sample_num)}
...
stan_v <- qpcr_mat[, observed_taxa, drop = FALSE]
mode(stan_v) <- "integer"
...
if (q == q_obs) {
  if (args$leave_one_out < 999) {
    naive_ests <- apply(matrix((q_obs+1):q), 1, naive_estimator, br16_mat[samp, , drop = FALSE], stan_v[samp, , drop = FALSE], observed_taxa)
    if (stan_data_lst$N == 1) {
      naive_ests <- matrix(naive_ests, nrow = 1)
    }
    naive <- cbind(stan_v[samp, , drop = FALSE], naive_ests)
  } else {
    naive <- stan_v[samp, , drop = FALSE]
  }
} else {
  naive_ests <- apply(matrix((q_obs+1):q), 1, naive_estimator, br16_mat[samp, , drop = FALSE], stan_v[samp, , drop = FALSE], observed_taxa)
  if (stan_data_lst$N == 1) {
...
set.seed(4747)
eps <- 1e-1
error_mu_tilde <- t(replicate(stan_data_lst$N, rnorm(args$q, 0, 10*eps)))
...
if (args$n_chains > 1) {
  if (args$n_chains == 4) {
    inits_list <- c(list(list(log_mu_tilde = log_mu_tilde),
                         list(log_Sigma = log(naive_sigma)),
                         list(init = "random"),
                         list(init = "random")))
  } else {
    inits_list <- c(list(list(log_mu_tilde = log_mu_tilde),
                         list(log_Sigma = log(naive_sigma))),
                    replicate(args$n_chains - 2, list(init = "random"), simplify = FALSE))
  }

} else {
  inits_list <- list(list(log_mu_tilde = log_mu_tilde, beta_0 = naive_beta, log_Sigma = log(naive_sigma)))
}
...
saveRDS(save_lst, paste0("/fh/fast/huang_y/user/bwillia2/qPCR/qpcr_data_analysis_est_", args$estimator, "_q_", args$q, "_q_obs_", q_obs, "_sample_", stan_data_lst$N, "_loo_", args$leave_one_out, "_as_", args$alpha_sigma, "_ks_", args$kappa_sigma, "_adjust_", ifelse(args$adjust, 1, 0), ".rds"))

trace_plot_nms <- c("mu", "beta_0", "Sigma")
    ...
    for (i in 1:args$q) {
      ...
      if (sum(logi) > 10) {
        for (j in 1:ceiling(sum(logi)/10)) {
          CairoPNG(paste0(trace_plots_dir, "/", args$estimator, "_par_", trace_plot_nms[n], "_taxon_", i, "_slice_", j, "_q_", args$q, "_q_obs_", q_obs, "_sample_", as.numeric(args$sample), "_loo_", args$leave_one_out, "_as_", args$alpha_sigma, "_ks_", args$kappa_sigma, ".png"), width = fig_width, height = fig_height, res = 300, units = "px")
          plot(traceplot(mod, pars = names(mod)[logi][1:10 + (j-1)*10]), cex = cex)
          dev.off()
        }
      } else {
        CairoPNG(paste0(trace_plots_dir, "/", args$estimator, "_par_", trace_plot_nms[n], "_taxon_", i, "_q_", args$q, "_q_obs_", q_obs, "_sample_", as.numeric(args$sample), "_loo_", args$leave_one_out, "_as_", args$alpha_sigma, "_ks_", args$kappa_sigma, ".png"), width = fig_width, height = fig_height, res = 300, units = "px")
