...
pred_cover <- performance_by_type %>%
  ggplot(aes(x = sqrt(numeric_sigma_beta), y = wald_pred_cover, 
             group = paste(as.character(grouping), "_", sigma_beta, sep = ""), fill = grouping)) +
  xlab(expression(sigma[beta])) +
  ylab(expression(Coverage)) + 
  labs(fill = expression(paste("Estimator; q"))) +
...
cred_cover <- performance_by_type %>%
  ggplot(aes(x = sqrt(numeric_sigma_beta), y = cover, 
             group = paste(as.character(grouping), "_", sigma_beta, sep = ""), fill = grouping)) +
  xlab(expression(sigma[beta])) +
  ylab(expression(Coverage)) + 
...
rmspe <- performance_by_type %>% 
  ggplot(aes(x = sqrt(numeric_sigma_beta), y = rmspe, 
             group = paste(as.character(grouping), "_", sigma_beta, sep = ""), fill = grouping)) +
  xlab(expression(sigma[beta])) +
  ylab(expression(RMSPE)) + 
  labs(fill = expression(paste("Estimator; q"))) +
  ggtitle("Root mean squared prediction error") +
  geom_boxplot(width = 0.8, position = position_dodge(width = 0.7, preserve = "total"), outlier.shape = NA) +
  scale_fill_manual(values = cols) +
...
rmse <- performance_by_type %>% 
  ggplot(aes(x = sqrt(numeric_sigma_beta), y = rmse, 
             group = paste(as.character(grouping), "_", sigma_beta, sep = ""), fill = grouping)) +
  xlab(expression(sigma[beta])) +
...
## actually plot them
png(paste0(plots_dir, "vary_sigma_beta", plot_nm, ".png"), width = fig_width, height = fig_height, units = "px", res = 300)
gridExtra::grid.arrange(cred_cover, pred_cover,
                        rmse, rmspe,
                        nrow = 2, ncol = 2)
dev.off()
...
width <- performance_by_type %>% 
  ...
  ggplot(aes(x = sqrt(numeric_sigma_beta), y = wald_width,
             group = paste(as.character(grouping), "_", sigma_beta, sep = ""), fill = grouping)) +
  xlab(expression(sigma[beta])) +
...
png(paste0(plots_dir, "vary_sigma_beta_width", plot_nm, ".png"), width = fig_width, height = fig_height, units = "px", res = 300)
width
dev.off()
...
bias <- performance_by_type_over_n %>% 
  ...
  xlab(expression(sigma[beta])) +
  ylab("Estimated bias") +
  ggtitle(expression(paste("Estimated bias for ", mu))) +
  geom_point(position = position_dodge(width = 0.7, preserve = "total")) +
  labs(color = "q") +
...
png(paste0(plots_dir, "vary_sigma_beta_bias", plot_nm, ".png"), width = fig_width, height = fig_height, units = "px", res = 300)
bias
dev.off()
## -----------------------------------------------------------------------------------------------------
...
png(paste0(plots_dir, "vary_sigma_beta_cover_vs_rank_order_40", plot_nm, ".png"), width = fig_width, height = fig_height, units = "px", res = 300)
gridExtra::grid.arrange(cover_vs_rank_order_q_40_sig_beta_1, cover_vs_rank_order_q_40_sig_beta_5,
...
png(paste0(plots_dir, "vary_sigma_beta_cover_vs_rank_order_20", plot_nm, ".png"), width = fig_width, height = fig_height, units = "px", res = 300)
gridExtra::grid.arrange(cover_vs_rank_order_q_20_sig_beta_1, cover_vs_rank_order_q_20_sig_beta_5,
...
png(paste0(plots_dir, "vary_sigma_beta_cover_vs_rank_order_10", plot_nm, ".png"), width = fig_width, height = fig_height, units = "px", res = 300)
gridExtra::grid.arrange(cover_vs_rank_order_q_10_sig_beta_1, cover_vs_rank_order_q_10_sig_beta_5,
